...
contract DeployScript is Script, HelperConfig {
    ...
    function deployMarkets() public {
        for (uint i = 0; i < configVariables.amountOfNewMarkets; ++i) {
            ...
            require(
                ...
                "marketId of markets and loop are not the same"
            );
    ...
    function deployEpochs() public {
        for (uint i = 0; i < configVariables.amountOfNewEpochs; ++i) {
            ...
            require(
                ...
                "marketId of epochs and loop are not the same"
            );
    ...
    function deployFarms() public {
        for (uint i = 0; i < configVariables.amountOfNewFarms; ++i) {
            ...
            require(
                farms.marketId == marketId,
                "marketId of farms and loop are not the same"
            );

            (address _rHedge, address _rRisk) = rewardsFactory
                .createStakingRewards(marketId, farms.epochEnd);

            fundFarms(
                _rHedge,
                _rRisk,
                farms.farmRewardsHEDGE,
                farms.farmRewardsRISK
            );
        }
    }
    ...
    function fundFarms(
        ...
        StakingRewards(_rRisk).notifyRewardAmount(
            stringToUint(_rewardsAmountRISK)
        );
